output "server_ca_cert_pem" {
  description = "Public certificate for the Server CA, to trust Vault"
...
output "server_ca_private_key_pem" {
  description = "Private key for the Server CA, to trust Vault"
  value     = tls_private_key.server_ca_key.private_key_pem
  sensitive = true
}

output "unseal_key_id" {
  value = aws_kms_key.unseal[0].arn
}